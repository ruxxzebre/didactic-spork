...
var ACRONYMS = ['ID', 'URL', 'JSON', 'HTML', 'PDF', 'IP', 'SMS', 'ISO', 'ZIP', 'AMP', 'ISP', 'OS', 'IOS', 'UTM', 'UTC', 'GDPR', 'API', 'VAT', 'IVR', 'MRR', 'PO'];
var ARTICLES = ['at', 'by', 'to', 'on', 'in', 'of', 'for', 'from', 'or', 'via', 'be', 'is'];
var MATCH_TYPES = {
...
var parseLabel = function parseLabel(name) {
  var regex = /(\d*[A-Z]*\d*[a-zA-Z]\d*[a-z]*)/g;
  var words = name.match(regex);
...
var parseMisc = function parseMisc(string) {
...
var processJSON = function processJSON(jsonObj) {
  ...
  if (withLabel) {
    itemConstructor = function itemConstructor(key, other) {
